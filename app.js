...
/* v5.3.29 — All Wozzas BACK control */
document.addEventListener('click', function(e){
  const back = e.target.closest && e.target.closest('.wozza-menu-back');
  if(!back) return;
  e.preventDefault();
  e.stopPropagation();

  const menu =
    back.closest('.wozza-menu') ||
    back.closest('.all-wozzas-menu') ||
    back.closest('[data-wozza-menu]');

  if(menu){
    menu.classList.remove('open','is-open','show','active');
    menu.setAttribute('aria-hidden','true');
  }

  const trigger =
    document.querySelector('[data-wozza-menu-trigger]') ||
    document.querySelector('.wozza-menu-trigger') ||
