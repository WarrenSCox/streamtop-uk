const KEY='wozzawatch-watched-v1',LIST='wozzawatch-my-list-v1',TYPES=['SHOW','MOVIE','BOOK'];let type='SHOW';const $=s=>document.querySelector(s),esc=s=>String(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));const get=k=>{try{return JSON.parse(localStorage.getItem(k)||'[]')}catch{return[]}},put=(k,v)=>localStorage.setItem(k,JSON.stringify(v));
