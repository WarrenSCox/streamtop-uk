...
const listEl=document.querySelector('#myList'), emptyEl=document.querySelector('#emptyList');
...
function render(){
 const items=readList(); listEl.innerHTML=''; emptyEl.classList.toggle('hidden',items.length>0);
 items.forEach((item,i)=>{
  ...
  if(item.poster){const img=document.createElement('img');img.className='poster';img.alt='';img.src=item.poster;img.loading='lazy';a.append(img)}else{const p=document.createElement('div');p.className='poster poster-placeholder';p.textContent='▶';a.append(p)}
  const info=document.createElement('div');info.className='item-info';const wrap=document.createElement('div');const title=document.createElement('div');title.className='title';title.textContent=item.title;const meta=document.createElement('div');meta.className='list-meta';meta.textContent=`${item.service||''}${item.type==='SHOW'?' · TV':' · Movie'}`;wrap.append(title,meta);info.append(wrap);
