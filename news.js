...
let active="UK", provider="SKY", data={categories:{},providers:{}};
...
const PROVIDER_META={
 SKY:{label:"SKY NEWS",url:"https://news.sky.com/info/rss"},
 GUARDIAN:{label:"THE GUARDIAN",url:"https://www.theguardian.com/help/feeds"},
 INDEPENDENT:{label:"THE INDEPENDENT",url:"https://www.independent.co.uk/service/rss-feeds-775086.html"}
};
function providerRows(){return data.providers?.[provider]?.categories?.[active]||((provider==="SKY")?(data.categories?.[active]||[]):[])}
function refreshProvider(){
 document.querySelectorAll(".news-provider").forEach(b=>b.classList.toggle("active",b.dataset.provider===provider));
 const meta=PROVIDER_META[provider];
 const badge=$("#newsSourceBadge"),name=$("#newsSourceName");
 if(name)name.textContent=meta.label;
 if(badge)badge.href=meta.url;
 render();
}
function initProviders(){
 document.querySelectorAll(".news-provider").forEach(b=>b.addEventListener("click",()=>{provider=b.dataset.provider;refreshProvider()}));
}
function render(){let rows=providerRows();$("#newsTitle").textContent="LATEST "+active;$("#newsChart").innerHTML=rows.slice(0,10).map((x,i)=>`<li class="news-row"><span class="rank">${String(i+1).padStart(2,"0")}</span><a class="news-image-link" href="${esc(x.link)}" target="_blank" rel="noopener" aria-label="${esc(x.title)}">${x.image?`<img class="poster news-thumb" src="${esc(x.image)}" alt="">`:`<span class="poster news-thumb news-thumb-fallback">W</span>`}</a><a class="news-story" href="${esc(x.link)}" target="_blank" rel="noopener"><span class="news-copy"><strong>${esc(x.title)}</strong><small>${ago(x.published)}</small></span></a></li>`).join("");$("#newsError").classList.toggle("hidden",rows.length>0)}
async function loadNews(){
 try{
  const r=await fetch("./news.json?v="+Date.now(),{cache:"no-store"});
  if(!r.ok)throw new Error("news.json "+r.status);
  const x=await r.json();
  data=(x&&(x.categories||x.providers))?x:{categories:{},providers:{}};
...
function initMenu(){const menu=$("#wozzaMenu"),bd=$("#wozzaMenuBackdrop"),trigger=$(".header-copy");let timer=null,start=null;
 ...
 document.addEventListener("pointerdown",e=>{if(e.clientX>innerWidth-32||e.target.closest("a,button,.wozza-menu"))return;start={x:e.clientX,y:e.clientY};timer=setTimeout(()=>{if(start&&start.x>innerWidth*.2&&start.x<innerWidth*.8&&start.y>innerHeight*.2&&start.y<innerHeight*.8)open()},350)});
