...
function tabs(){let n=$("#newsTabs");CATS.forEach((c,i)=>{let b=document.createElement("button");b.textContent=c;b.style.background=COLORS[i];b.className=c===active?"active":"";b.onclick=()=>{active=c;tabsRefresh();render()};n.appendChild(b)})}
function tabsRefresh(){[...$("#newsTabs").children].forEach(b=>b.classList.toggle("active",b.textContent===active))}
